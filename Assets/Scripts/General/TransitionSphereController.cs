...
public class TransitionSphereController : MonoBehaviour
{
    ...
    {
        _meshRenderer.material.DOFloat(BLACK_VALUE, "_Offset", 1.0f).From(-1.0f).SetEase(Ease.InCubic).OnComplete(() => OnFadeToBlackCompleted?.Invoke());
        _isBlack = true;
    }
    ...
    {
        _isBlack = true;
        yield return _meshRenderer.material.DOFloat(BLACK_VALUE, "_Offset", 1.0f).From(-1.0f).SetEase(Ease.InCubic).WaitForCompletion();
    ...
    {
        _isBlack = false;
        yield return _meshRenderer.material.DOFloat(WORDL_VALUE, "_Offset", 1.0f).From(0f).SetEase(Ease.InCubic).WaitForCompletion();
        OnFadeToWorldCompleted?.Invoke();
    }

    public void FadeToWorld()
    {
        _meshRenderer.material.DOFloat(WORDL_VALUE, "_Offset", 1.0f).From(0f).SetEase(Ease.InCubic).OnComplete(() => OnFadeToWorldCompleted?.Invoke());
        _isBlack = false;
    }
