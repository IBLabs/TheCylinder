...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRPushButton : XRBaseInteractable
    {
        ...
        void SetButtonHeight(float height)
        {
            ...
            Debug.Log("[TEST]: new local position: " + m_Button.localPosition);
        }
        ...
        {
            foreach (var (key, pressInfo) in m_HoveringInteractors)
            {
                if (key is XRDirectInteractor directInteractor)
                {
                    directInteractor.SendHapticImpulse(0.1f, 0.05f);
                }
            }
